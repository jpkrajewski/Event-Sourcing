...
from facades.blockchain_facade import BlockChainFacade
from facades.transfer_facade import TransferFacade
from facades.tank_facade import TankFacade
...
class Facade(AccountFacade, BlockChainFacade, TransferFacade, TankFacade):
    ...
    def _check_required_attributes(self):
        for facade in inspect.getmro(self.__class__):
            if isinstance(
                facade, object
            ):  # MRO returns object as last class, we don't need it
                break
            for required in facade.required_attributes:
                if not hasattr(self, required):
                    raise AttributeError(
                        f"Facade has no attribute self.{required} required by {facade.__name__}."
                    )

    def _map_functions_to_numbers(self):
        self.mapped = {}
        facades_functions = [
            name for name in dir(self) if self._is_function_from_facade(name)
        ]
        sorted_facades_functions = sorted(
            facades_functions, key=lambda x: getattr(self, x).__doc__
        )
